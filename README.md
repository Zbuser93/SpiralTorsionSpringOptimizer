...
If we keep the spring's radius at the open state the same, then changing the spring's preload torque will (among other things) effect the radius at rest-- therefore if we want to include the spring's preload torque into the optimization, we need to use the open state radius as a constraint and not just optimize the spring at rest.
